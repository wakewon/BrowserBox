...
CONTAINER_ID=$($SUDO docker run --cap-add=sys_nice -v $HOME/sslcerts:/home/bbpro/sslcerts -d --name browserbox -p $PORT:$PORT -p $(($PORT-2)):$(($PORT-2)) -p $(($PORT-1)):$(($PORT-1)) -p $(($PORT+1)):$(($PORT+1)) -p $(($PORT+2)):$(($PORT+2)) --cap-add=SYS_ADMIN "${DOCKER_IMAGE_WITH_TAG}" bash -c 'source ~/.nvm/nvm.sh; pm2 delete all; sudo chown bbpro:bbpro ~/sslcerts/*; echo $(setup_bbpro --port '"$PORT"' --token '"$5"') > login_link.txt; ( bbpro || true ) && tail -f /dev/null')
